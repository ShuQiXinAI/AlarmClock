...
### vivo（OriginOS 4 / X100 Pro 实测）
**最小必要操作：在最近任务页长按本 App 卡片 → 点击 🔒 锁定**。
仅此一步即可保证锁屏 / 后台到点正常响铃，**无需**单独配置自启动、关联启动、后台活动、电池优化等权限。
原因：vivo 的"卡片锁定"权重高于其他后台管理策略，被锁定的 App 不会被系统冻结。
...
### v1.2.0（权限引导 + 国产 ROM 适配）
- ✨ **权限自检页**：列出通知权限、精确闹钟权限当前状态，一键跳转对应系统设置页
- ✨ **首次启动引导**：第一次打开 App 自动跳到权限自检页，避免用户错过关键设置
- ✨ **测试闹钟按钮**：30 秒后触发测试闹钟，快速验证权限链是否打通
- ✨ **国产 ROM 自动识别**：检测 vivo / 小米 / 华为 / 荣耀 / OPPO / realme / 三星，**显示该品牌特定的设置步骤**（vivo 实测仅需"锁定卡片"一步即可）
- 🎨 首页"通知设置"按钮改名为"权限"，跳转到自检页
